...
def generate_master_password_suggestion(
    style: str = "eff",
) -> tuple[str, str]:
    """
    Generate a master password suggestion for display at forge time.

    Returns (password, source) where source is:
      "keepassxc-cli"  — generated by KeePass's own CSPRNG (when available)
      "eff"            — EFF diceware-style "word-word-word-word" (default)
      "secrets"        — Capital.word.phrase.9 via Python secrets module

    Args:
      style: "eff" (default) — diceware-style, most memorable
             "classic"       — Capital.word.phrase.9 format
             "keepassxc"     — try keepassxc-cli first, fall back to secrets

    Investigation note (2026-06-02): keepassxc-cli's `generate` command does
    not support word-based / diceware passphrases. The GUI has a diceware
    plugin but it is not exposed via the CLI. This module therefore implements
    its own EFF-style generator as the default for master password suggestions.

    Both "eff" and "secrets" use os.urandom (CSPRNG) as the entropy source.
    """
    if style == "keepassxc":
        ...
        # Fall through to classic format
    ...
    if style == "eff":
        try:
            ...
            from passphrase_eff import generate_eff_passphrase
            return generate_eff_passphrase(word_count=4), "eff"
        except ImportError:
            pass

    # "classic" or EFF import failed
    passphrase = generate_passphrase()
    return passphrase, "secrets"
